<?php
	...
	if (isset($_POST['search'])) {  
			...
			if($returnedResults > 0){
				...
				echo("<input type='number' name='seatsRequired' placeholder='How many seats would you like to book?'
				min='1' max='66' style='width:100%;margin:4px;height:10px;'>");
				...
				$error=("Sorry, no results matched your criteria.");
			}
		...
		if (($seatsRequired) > 0){
			...
			if ($seatsRequired <= $remainingSeats){
				...
				$success = ("Your booking of $seatsRequired seats to watch a performance on the $selectedBookingDate is complete, your booking reference number is #$bookingNo.
				 Your payment method has been charged $$totalPrice.");
			} else {
				$error = ("Can't book $seatsRequired seats. Only $remainingSeats seats remain for this performance. Please try again with less.");
			}
		}
	}
