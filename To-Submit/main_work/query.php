<?php
	...
	if (isset($_POST['search'])) {  
			...
			SELECT
				...
				C.cinemaNo AS CinemaNo, S.price AS price, A.seatsRemain AS seatsRemain
			FROM
			...
			if($returnedResults > 0){
				...
				for ($i = 1; $i <= $returnedResults; $i++){
					echo("<input type='submit' name ='resultButton' class='resultbutton' value='Book Result $i  Now'>");
				}
		...
		if (($seatsRequired) > 0){
			...
			$remainingSeats = mysql_result(mysql_query("SELECT seatsRemain FROM temp WHERE Row_Num = $value"),0);
			if ($seatsRequired <= $remainingSeats){
				...
				echo($remainingSeats);
				echo($seatsRequired);
				echo(($remainingSeats-$seatsRequired));
